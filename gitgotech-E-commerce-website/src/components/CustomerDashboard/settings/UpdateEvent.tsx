...
import { useUpdateEventMutation, useGetEventsQuery } from '@/redux/features/event/eventSlice';
...
export default function UpdateEvent() {
  ...
  // Fetch events to get the current event data
  ...
  useEffect(() => {
    if (eventId && eventsData?.data?.data) {
      const event = eventsData.data.data.find((e: any) => e._id === eventId);
      if (event) {
        ...
        setFormData({
          ...
          purchaseOption: event.purchaseOption || 'creator',
  ...
  return (
    <div className="min-h-screen mt-12 md:mt-20 bg-gradient-to-r from-black via-[#0f0924] to-black p-6">
      {/* Header */}
      <div className="container mx-auto flex items-center gap-4">
        ...
        <h1 className="text-[32px] font-semibold text-gray-300 font-cormorant">Update Event</h1>
      </div>
          ...
          <div className="space-y-3">
            ...
            <label className="flex items-center gap-3 cursor-pointer">
              <input
                ...
                name="purchaseOption"
                value="participator"
                checked={formData.purchaseOption === 'participator'}
                onChange={handleChange}
                className="w-4 h-4 text-blue-600 bg-gray-800 border-gray-600 focus:ring-blue-500"
              />
