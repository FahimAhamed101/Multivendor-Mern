...
import { useAddShowroomMutation } from '@/redux/features/vendor/showroomSlice/showroomSlice';
...
export default function AddShowroom() {
  ...
  const [addshoroom] = useAddShowroomMutation();
  ...
  // Convert HH:MM to ISO using fixed base date 2026-01-31
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const res = await addshoroom(fd).unwrap();
      if (res?.success) {
        toast.success(res?.message || 'Showroom created successfully');
        router.push('/v-profile');
      }
    } catch (error: any) {
      const msg =
        error?.data?.message || error?.data?.data || 'Failed to create showroom';
      toast.error(typeof msg === 'string' ? msg : 'Failed to create showroom');
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gradient-to-r from-black via-[#0f0924] to-black text-white p-4 md:p-8">
      ...
      <div className="max-w-4xl flex items-center gap-2 mx-auto mt-8 md:mt-20">
        ...
        <h1 className="text-2xl font-semibold text-gray-300 font-cormorant">Add New Showroom</h1>
      </div>
      ...
      <form onSubmit={handleSubmit} className="max-w-4xl mx-auto mt-6 space-y-8">
        ...
        <div className="bg-gradient-to-br from-purple-900/30 via-gray-900/80 to-purple-900/20 border border-purple-500/30 rounded-2xl p-6 md:p-8 backdrop-blur-sm">
          ...
          {showMap && (
            <p className="text-xs text-gray-500 mt-2">Click anywhere on the map to set location</p>
          )}
        </div>

        {/* Documents */}
        <div className="bg-gradient-to-br from-purple-900/30 via-gray-900/80 to-purple-900/20 border border-purple-500/30 rounded-2xl p-6 md:p-8 backdrop-blur-sm">
          <h2 className="text-lg font-semibold text-gray-300 mb-4">Documents</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <FileInput
              label="NID Image"
              file={nidImage}
              onChange={(e) => handleFileChange(setNidImage, e)}
            />
            <FileInput
              label="Owner Image"
              file={ownerImage}
              onChange={(e) => handleFileChange(setOwnerImage, e)}
            />
          </div>
        </div>
        ...
        <div className="flex justify-center mt-8 pb-8">
          <button
            type="submit"
            className="inline-flex items-center gap-2 px-8 py-3 bg-purple-600 hover:bg-purple-700 rounded-full text-white font-medium text-lg transition-all duration-300 transform hover:scale-[1.02] shadow-lg shadow-purple-500/20"
          >
