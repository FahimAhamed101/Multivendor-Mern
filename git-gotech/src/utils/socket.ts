...
export const initSocketIO = async (server: HttpServer) => {
  ...
  io.on("connection", (socket: Socket) => {
    if (socket.user && socket.user._id) {
      ...
      logger.info("Socket just connected:", {
        socketId: socket.id,
    ...
    socket.on("sendMessage", (data: { receiverId: string; message: any }) => {
      const receiverSocket = connectedUsers[data.receiverId];
      if (receiverSocket) {
        io.to(receiverSocket.socketID).emit("receiveMessage", data.message);
      }
    });
