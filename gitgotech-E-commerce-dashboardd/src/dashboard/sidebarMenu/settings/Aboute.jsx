...
import { useGetAboutUsQuery } from "../../../redux/features/settings/settingsSlice";

const AbouteUs = () => {
  const navigate = useNavigate();
  const { data: aboutData, isLoading, isError, refetch } = useGetAboutUsQuery();
  
  const content = aboutData?.data?.value || "";
  const lastUpdated = aboutData?.data?.updatedAt || aboutData?.data?.createdAt;
      ...
      {isError && !isLoading && (
        <div className="flex items-center justify-center py-20">
          <div className="text-center">
            <p className="text-red-400 text-lg mb-2">Failed to load about us</p>
            <button
              onClick={refetch}
        ...
        <div className="relative rounded-xl overflow-hidden border border-gray-800 mb-8">
          ...
          <div className="p-6 bg-[#0f0c11]">
            {content ? (
              ...
              <p className="text-gray-400 text-center py-8">No about us content available.</p>
            )}
          </div>
        </div>
      )}
